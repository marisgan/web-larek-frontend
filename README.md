...
Методы:
- inputChange - инициирует событие инпута
- set valid - управляет активностью кнопки сабмита
- set errors - отображает текст обибок под полем ввода
...
Поля:
- totalPay - элемент разметки для вывода суммы покупки
- goShopping - элемент разметки для кнопки перехода и закрытия модального окна
...
- `preview:select` - выбор карточки для отображения в модальном окне
- `cart:add` - добавление товара в козину
- `cart:delete` - удаление товара из корзины
- `cart:open` - открытие корзины
- `order:open` - открытие окна с формой сбора данных оплаты и доставки
...
- `succsess:closed` - закрытие финального окна